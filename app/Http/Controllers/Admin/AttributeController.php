...
class AttributeController extends Controller
{
    ...
    public function update(UpdateAttributeRequest $request, Attribute $attribute)
    {
	   ...
	   $saveattr				 = Attribute::where('id',$attribute->id)->first();
	   $saveattr->name			 = $request->get('name');
	   $saveattr->description	 = $request->get('description');
       $saveattr->save();
        ...
        if(count($idvalues) > 0){
            foreach($datas as $data){
                foreach($idvalues as $val){
                    if($val !== $data->value){
                        \DB::table('attribute_values')->where('id', $data->id)->delete();
