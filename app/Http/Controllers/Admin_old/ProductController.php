...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('front_image', function ($row) {
                ...
                $count = !empty($varients) ? $varients->count() : 0;
                $img   = (isset($image->file_name) && $image->file_name!="") ? $image->file_name : "";
                return '<div class="text-center"><img style="width : 50px; height : 50px;border : 1px solid lightgrey;" src="'.asset("file/".$img).'"></div>';
    ...
    public function edit(Product $product)
    {
       ...
       // echo "<pre>"; print_r($mapAttributes->colors); die;
...
*/
       ...
       // ProductAttribute::destroy($acollection->toArray());
        ...
        ProductAttribute::where('product_id',$product->id)->forceDelete();
                    ...
                    if ($key && isset($value['sizes'])) {
                        foreach($value['sizes'] as $k=> $v){
                            ...
                            ProductVariation::create([
                               ...
                               'product_id' => $product->id,
                               'status' => 1,
                               'primary_variation'=> (isset($request->primary) && array_key_exists((int)$key,$request->primary)) ? 1 : 0
                           ]);
                        }
                    }
                }
