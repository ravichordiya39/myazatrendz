...
class HomeCntroller extends Controller
{
    public function index() {
        ...
        // ->leftJoin('product_images', function ($leftJoin) {
        //     $leftJoin->on('products.id', '=', 'product_images.product_id')
        //          ->where('product_images.type', '=', 1)
        //          ->where('product_images.id', '=', DB::raw("(select max(`id`) from product_images)"));
        // })
        // ->where('products.is_exclusive',1)
        // ->where('products.status',1)
        // ->orderBy('products.created_at', 'desc')
        // ->select('products.*', 'categories.name as category_name', 'product_images.file_name')
        // ->paginate(20);
        ...
        // dd(session()->get('cart'));
        // exit();
        return view('frontend.home',compact('banners','trending','hotesale','latest','newarrivals','bestsellers'));
    }
    ...
    if ($request->ajax()) {
     ...
     // colors
      ...
      if($request->has('attributes')){
       \Log::info($request->attributes[0]);
        // \Log::info($request->attributes->toArray());
