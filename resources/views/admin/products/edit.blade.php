                ...
                <div class="col-2">
                        <div class="icheck-primary {{ $errors->has('same_price') ? 'is-invalid' : '' }}" style="display : inline-block";>
                            <input type="checkbox" name="same_price[]" id="same_price{{ $color->id }}"
                                ...
                                value="{{ $loop->iteration }}" {{ (isset($product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->single_price))? "checked" : '' }} style="display: inline-block;">
                    ...
                    <div class="form-group col-2">
                        <input class="form-control"
                            ...
                            value="{{ old('single_price_'.$color->id,$product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->single_price??'') }}" onblur="txtSinglePrice('{{ $color->id }}')" onkeypress="return isFloat(event)"
                            maxlength="10" placeholder="{{ trans('cruds.product.fields.single_price') }}" {{ (isset($product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->single_price))? "" : 'disabled' }}>
                    </div>
                    <div class="form-group col-2">
                        <input class="form-control"
                            type="text" name="single_price_quantity_{{ $color->id }}" id="txtSinglePrice_quantity_{{ $color->id }}"
                            value="{{ old('single_price_quantity_'.$color->id,$product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->single_price_quantity??'') }}" onblur="txtSinglePriceQuantity('{{ $color->id }}')" onkeypress="return isFloat(event)"
                            maxlength="10" placeholder="Single Price Qty"  {{ (isset($product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->single_price_quantity))? "" : 'disabled' }}>
                    </div>
                    <div class="form-group col-2">
                        <input class="form-control txtWholesalePrice"
                            type="text" name="wholesale_price_{{ $color->id }}" id="txtWholesalePrice_{{ $color->id }}"
                            value="{{ old('wholesale_price_'.$color->id,$product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->wholesale_price??'') }}"  onblur="txtWholesalePrice('{{ $color->id }}')" onkeypress="return isFloat(event)"
                            maxlength="10" placeholder="{{ trans('cruds.product.fields.wholesale_price') }}"  {{ (isset($product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->wholesale_price))? "" : 'disabled' }}>
                    </div>
                    <div class="form-group col-2">
                        <input class="form-control"
                            type="text" name="wholesale_price_quantity_{{ $color->id }}" id="txtWholesalePrice_quantity_{{ $color->id }}"
                            value="{{ old('wholesale_price_quantity_'.$color->id,$product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->wholesale_price_quantity??'') }}" onblur="txtWholesalePriceQuantity('{{ $color->id }}')" onkeypress="return isFloat(event)"
                            maxlength="10" placeholder="Wholesale Price Qty" {{ (isset($product->productProductVariations->where('color_id',$color->id)->where('product_id',$product->id)->first()->wholesale_price_quantity))? "" : 'disabled' }}>
                    </div>
                    <div class="col-1">
                        {{-- <div class="icheck-primary {{ $errors->has('same_price') ? 'is-invalid' : '' }}">
                            <input type="checkbox" name="same_price[]" id="same_price{{ $color->id }}"
                                ...
                                @foreach ($sizes as $key => $size)
                                    ...
                                    <div class="form-group col-2">
                                        <input class="form-control single_price_quantity_{{ $color->id }} {{ $errors->has('single_price_quantity') ? 'is-invalid' : '' }}"
                                            ...
                                            id="singleQty_{{$loop->iteration }}_{{$size->id}}"
                                            >
                                    </div>
                                    ...
                                    <div class="form-group col-2">
                                        <input class="form-control wholesale_price_quantity_{{ $color->id }} {{ $errors->has('wholesale_price_quantity') ? 'is-invalid' : '' }}"
                                            ...
                                            onkeypress="return isFloat(event)" maxlength="8"
                                            id="wholesaleQty_{{$loop->iteration }}_{{$size->id}}"
                                            >
                                    </div>
